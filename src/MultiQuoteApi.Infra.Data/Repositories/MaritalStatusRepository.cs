...
{
    internal class MaritalStatusRepository(MultiQuoteDbContext context) : DomainRepository<MaritalStatus>(context), 
        IMaritalStatusRepository
    {
        public async Task<IEnumerable<MaritalStatus>?> GetAllAsync(RecordStatusEnum recordStatus)
        {
            var query = GenerateQuery(
                            filter: (filtr => filtr.Status.Equals((int)recordStatus)),
                            orderBy: item => item.OrderBy(y => y.MaritalStatusId));
